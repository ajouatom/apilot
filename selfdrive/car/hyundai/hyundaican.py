...
def create_acc_commands_mix_scc(CP, packer, enabled, accel, upper_jerk, idx, hud_control, set_speed, stopping, CC, CS):
  ...
  if makeNewCommands:
    ...
    "SCCInfoDisplay" : 4 if longEnabled and softHold else 3 if longEnabled and radarAlarm else 2 if enabled else 0,   #2: 크루즈 선택, 3: 전방상황주의, 4: 출발준비
    ...
    values["SCCInfoDisplay"] = 4 if longEnabled and softHold else 3 if longEnabled and radarAlarm else 2 if enabled else 0   #2: 크루즈 선택, 3: 전방상황주의, 4: 출발준비
    values["ObjValid"] = 1
