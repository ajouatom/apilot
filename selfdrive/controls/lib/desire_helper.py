...
class DesireHelper:
  ...
  def detect_road_edge_dragonpilot(self, md):
  ...
  def update(self, carstate, lateral_active, lane_change_prob, md, turn_prob, navInstruction, roadLimitSpeed, lane_width):
    ...
    turning = abs(carstate.steeringAngleDeg) >= 20
    ...
    if self.autoTurnControl > 0:
      nav_direction, nav_turn, need_torque, nav_event, nav_distance = self.nav_update(carstate, navInstruction, roadLimitSpeed, road_edge_stat)
    else:
      nav_direction = LaneChangeDirection.none
      nav_turn = False
      nav_event = 0
