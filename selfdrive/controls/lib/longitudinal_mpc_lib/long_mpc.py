...
# 전방차량과의 상대속도에 의한 j,a cost변경
  ...
  def update(self, carstate, radarstate, model, controls, v_cruise, x, v, a, j, y, prev_accel_constraint):
    ...
    if self.mode == 'acc':
      ...
      if self.e2eMode:
        probe = model.stopLine.prob if abs(y[N])<3.0 else 0.0 # 모델이 직선도로가 아닌경우 정지하지 않음. probe를 0로
        startSign = v[-1] > 5.0
        #stopSign = model_x < 120.0 and ((v[-1] < 3.0) or (v[-1] < v_ego*0.95)) and abs(y[N]) < 3.0 #직선도로에서만 감지하도록 함~
        #stopSign = ((v[-1] < 3.0) or (v[-1] < v_ego*0.80)) and abs(y[N]) < 20.0 #직선도로에서만 감지하도록 함~
      ...
      v_cruise_clipped = np.clip(v_cruise * np.ones(N+1),
                                 v_lower,
                                 v_upper)
      cruise_obstacle = np.cumsum(T_DIFFS * v_cruise_clipped) + get_safe_obstacle_distance(v_cruise_clipped, self.t_follow, COMFORT_BRAKE, self.stopDistance)
      
      x_obstacles = np.column_stack([lead_0_obstacle, lead_1_obstacle, cruise_obstacle, x2])

      str1 = 'Y{:3.1f},TR={:.2f},state={} {},prob={:2.1f},L{:3.1f} C{:3.1f} X{:3.1f} S{:3.1f},V={:.1f}:{:.1f}:{:.1f}:{:.1f}'.format(
        y[N], self.t_follow, self.xState, self.e2ePaused, model.stopLine.prob, lead_0_obstacle[0], cruise_obstacle[0], x[N], stopline_x, v_ego, v[0], v[1], v[-1])
      self.debugText = str1
