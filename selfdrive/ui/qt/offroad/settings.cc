...
CommunityPanel::CommunityPanel(QWidget* parent) : QWidget(parent) {
  ...
  toggleLayout->addWidget(horizontal_line());
  toggleLayout->addWidget(new CValueControl("AutoNaviSpeedCtrl", "과속카메라작동 방법(2)", "0:사용안함, 1:NDA", "../assets/offroad/icon_road.png", 0, 1, 1));
...
TuningPanel::TuningPanel(QWidget* parent) : QWidget(parent) {
    ...
    toggleLayout->addWidget(new CValueControl("LongitudinalTuningKiV", "롱컨: I게인(200)", "(시험용) ", "../assets/offroad/icon_road.png", 0, 2000, 5));
    ...
    toggleLayout->addWidget(new CValueControl("StopAccelApply", "정지유지브레이크-2.0x(30%)", "정지유지시 브레이크압을 조정합니다. 0: 사용안함. ", "../assets/offroad/icon_road.png", 0, 100, 10));
    toggleLayout->addWidget(horizontal_line());
    ...
    toggleLayout->addWidget(new CValueControl("ApplyDynamicTFollow", "차량간격제어:상대속도-(105%)", "선행차와 점점 가까와지면 차량거리를 안전하게 증가시키도록 합니다.", "../assets/offroad/icon_road.png", 100, 300, 1));
    ...
    toggleLayout->addWidget(new CValueControl("ApplyDynamicTFollowDecel", "차량간격제어:감속(105%)", "차량이 급감속 할 수록 차량간격을 벌리도록 제어합니다.", "../assets/offroad/icon_road.png", 100, 300, 1));
    ...
    toggleLayout->addWidget(new CValueControl("ApplyModelDistOrder", "신호정지 감속모델선택 (29)", "숫자가적을수록 미리감속하고 서서히 정지합니다.", "../assets/offroad/icon_road.png", 1, 32, 1));
    ...
    toggleLayout->addWidget(new CValueControl("CruiseMaxVals1", "가속설정:0km/h(200)", "속도별 가속도를 지정합니다.(x0.01m/s^2)", "../assets/offroad/icon_road.png", 1, 250, 5));
    toggleLayout->addWidget(new CValueControl("CruiseMaxVals2", "가속설정:40km/h(150)", "속도별 가속도를 지정합니다.(x0.01m/s^2)", "../assets/offroad/icon_road.png", 1, 250, 5));
    toggleLayout->addWidget(new CValueControl("CruiseMaxVals3", "가속설정:60km/h(50)", "속도별 가속도를 지정합니다.(x0.01m/s^2)", "../assets/offroad/icon_road.png", 1, 250, 5));
    toggleLayout->addWidget(new CValueControl("CruiseMaxVals4", "가속설정:80km/h(20)", "속도별 가속도를 지정합니다.(x0.01m/s^2)", "../assets/offroad/icon_road.png", 1, 250, 5));
    toggleLayout->addWidget(new CValueControl("CruiseMaxVals5", "가속설정:110km/h(15)", "속도별 가속도를 지정합니다.(x0.01m/s^2)", "../assets/offroad/icon_road.png", 1, 250, 5));
    ...
    toggleLayout->addWidget(new CValueControl("AutoLaneChangeSpeed", "자동차선변경:속도(30)", "해당속도 이상에서만 자동차선변경", "../assets/offroad/icon_road.png", 5, 60, 5));
...
CruisePanel::CruisePanel(QWidget* parent) : QWidget(parent) {
    ...
    // 크루즈
    toggleLayout->addWidget(new CValueControl("CruiseControlMode", "크루즈연비제어", "목표속도를 일시적으로 올림", "../assets/offroad/icon_road.png", 0, 10, 1));
    toggleLayout->addWidget(new CValueControl("AutoSyncCruiseSpeedMax", "자동속도업데이트속도 (120)", "가속시 크루즈속도보다 높아지면 지정 속도를 올려줍니다.", "../assets/offroad/icon_road.png", 0, 140, 10));
    ...
    toggleLayout->addWidget(new CValueControl("MyEcoModeFactor", "드라이브모드:연비가속비율(80%)", "일반모드대비 가속비율을 지정합니다.", "../assets/offroad/icon_road.png", 10, 95, 5));
    toggleLayout->addWidget(new CValueControl("MySafeModeFactor", "드라이브모드:안전비율(80%)", "가속도, 정지거리, 감속율, 차간거리를 평소보다 안전하게 유지합니다.", "../assets/offroad/icon_road.png", 10, 90, 10));
