...
* GAP설정: 속도에 따른 자동차간거리,
  * 1단계: E2E모드, 연비절감모드,
  * 2단계: E2E모드, 연비절감모드2: GAS페달에서 떼면 관성주행, 단,전방레이더 감지시 크루즈주행,
  * 3단계: E2E모드, 일반주행,
  * 4단계: 일반모드
* 자동 E2E 전환: => 기능삭제 (항상E2E)
  * 인게이지: E2E모드 시작,
  * GAS페달 60%이상: E2E OFF,
...
* 자동 인게이지
  ...
  * 신호등이 감지되면 (버튼으로 크루즈 해지된 상태에서만...)
  * 브레이크에서 발을 떼었을때(핸들20도 이내)
    * 직선도로, 100M이내 정지선, 60km/h이하,
    * 전방에 차량이 있을때
    * 전방에 차량이 없지만, 속도가 40km/h이상일때 (현재속도로 크루즈 변경)
    * 정지상태에서 전방에 차량이 있을때
    * 주행중 선행차량이 30M(파라미터)이상 잡혔을때
* E2E(신호감지)모드
  * 예외상황
